...
//author：只会写bug的承泽
//编辑于07/04/2018凌晨， 游戏版本build0614正式版。
//感谢芝麻fu的测试协助。

package SampleMod;//包名，请根据自己的包路径修改，一般在创建类的时候自动填好。
...
public class ModBasePower_ByChengze extends AbstractPower{
	
	public static final String POWER_ID = "ModBasePower";//能力的ID，判断有无能力、能力层数时填写该Id而不是类名。
	public static final String NAME = "能力";//能力的名称。
	
	public static final String DESCRIPITON = "能力描叙";//不需要调用变量的文本描叙，例如钢笔尖（PenNibPower）。
	public static final String[] DESCRIPTIONS = {"","",""};//需要调用变量的文本描叙，例如力量（Strength）、敏捷（Dexterity）等。
	//以上两种文本描叙只需写一个，更新文本方法在第36行。
	
	public ModBasePower_ByChengze(AbstractCreature owner, int amount) {//参数：owner-能力施加对象、amount-施加能力层数。在cards的use里面用ApplyPowerAction调用进行传递。
		this.name = NAME;
		this.ID = POWER_ID;
		this.owner = owner;
		this.amount = amount;
